    ...
    <div class="p-12 shadow bg-white rounded-lg">
        <div class="pb-1 border-b">
            <div class="flex justify-between">
                <h1 class="color-primary text-xl font-bold">Edit Track</h1>
                <div class="flex md:pl-5 items-center cursor-pointer relative">
                    <nuxt-link :to="`/seller/tracks`" class="py-2 px-8 bg-primary text-white">
                        Back
                    </nuxt-link>
                </div>
            </div>
        ...
        <form @submit.prevent="onSubmit">
            <div class="grid lg:grid-cols-3 gap-4">
                <div class="py-4">
                    ...
                    <h2 class="px-5 py-2"> Audio file: <a class="text-blue-500" target="_blank" :href="imgurl + 'storage/'+audio_file" download>Audio</a></h2>
                </div>
                <div class=" py-4">
                    ...
                    <h2 class="px-5 py-2"> Zip file: <a class="text-blue-500" target="_blank" :href="imgurl + 'storage/'+zip_files" download>Zip</a></h2>
                </div>
                <div class="py-4">
                    ...
                    <img v-if="feature_image != null" :src="imgurl + 'storage/'+ feature_image" width="100" class="rounded-lg w-20 h-20  mx-5 my-2" height="50" alt="">
                </div>
            </div>
            <div class="grid lg:grid-cols-2 gap-5 lg:gap-10 ">
                ...
                <div class="px-2">
                    ...
                    <div class="wrapper__input pb-8">
                        ...
                        <input type="number" min="1" v-model="formdata.basic_price" required placeholder="$" :class="{ 'border-red-500': errors.basic_price }" name="basic_price" id="basic_price" class="number__appearance  px-5 py-2 border border-gray-400 focus:outline-none text-gray-600 rounded-full w-full">
                    ...
                    <div class="wrapper__input pb-8">
                        ...
                        <input type="number" min="1" placeholder="$" v-model="formdata.premium_price" :class="{ 'border-red-500': errors.premium_price }" name="premium_price" id="premium_price" class="px-5 py-2 border number__appearance  border-gray-400 focus:outline-none text-gray-600 rounded-full w-full">

                    </div>
                </div>
            </div>
            <div class="flex justify-center">
                <button type="submit" class="px-10 py-2 btn-bg rounded-full text-white" :disabled="btn">Update</button>
                <button type="button" class="px-10 py-2 border border-red-700 rounded-full text-red-600 ml-3" @click="deleteItem(track_id)">Delete</button>
            </div>
        </form>
    </div>
...
export default {
    ...
    watch: {
        $data: {
            handler: function (val, oldVal) {
                if (val.formdata.audio_file != '' && val.formdata.zip_files != '' && val.formdata.feature_image != '') {
                    if (val.audiocomplete && val.zipcomplete && val.imgcomplete) {
                        this.$router.push('/seller/tracks');
                    }
                } else if (val.formdata.audio_file != '' && val.formdata.zip_files != '') {
                    if (val.audiocomplete && val.zipcomplete) {
                        this.$router.push('/seller/tracks');
                    }
                } else if (val.formdata.zip_files != '' && val.formdata.feature_image != '') {
                    if (val.zipcomplete && val.imgcomplete) {
                        this.$router.push('/seller/tracks');
                    }
                } else if (val.formdata.audio_file != '' && val.formdata.feature_image != '') {
                    if (val.audiocomplete && val.imgcomplete) {
                        this.$router.push('/seller/tracks');
                    }
                } else if (val.formdata.audio_file != '') {
                    if (val.audiocomplete) {
                        this.$router.push('/seller/tracks');
                    }
                } else if (val.formdata.zip_files) {
                    if (val.zipcomplete) {
                        this.$router.push('/seller/tracks');
                    }
                } else if (val.formdata.feature_image != '') {
                    if (val.imgcomplete) {
                        this.$router.push('/seller/tracks');
                    }
                }
    ...
    methods: {
        ...mapActions({
            deleteFromEditTrack: 'seller/tracks/deleteFromEditTrack'
        }),
        audiofileAdded() {
        ...
        onSubmit() {
            ...
            let vm = this;
            this.$axios.put(
                `seller/tracks/update/${this.track_id}`,
                vm.formdata
            ).then(({
                data
            }) => {
                this.track_id = data.id
                if (this.formdata.audio_file != '') {
                    this.$refs.audio.dropzone.processQueue()
                }
                if (this.formdata.feature_image != '') {
                    this.$refs.image.dropzone.processQueue()
                }
                if (this.formdata.zip_files != '') {
    ...
    async asyncData({
        ...
        const {
            data
        } = await $axios.get(`/seller/tracks/${params.id}/edit`);
        if (data.success == true) {
            return {
                formdata: {
                    ...
                    genre: data.track.genre_id,
                    date: data.track.date,
                    bpm: data.track.bpm,
                    basic_price: data.track.basic_price,
                    premium_price: data.track.premium_price,
                },
                audio_file: data.track.audio_file,
                zip_files: data.track.deliverable_audio,
                feature_image: data.track.image,
                track_id: data.track.id
            }

        } else {
            return redirect('/seller/tracks/');
        }
    }
}
</script>
